...
export default function FormDialog({ text, id, desc, time, indexList, usersCard, index }) {
  ...
  // const trueUsers = users.map((user, index) => user.userId === usersCard[index])
  console.log(cUsers)
  ...
  const remove = (id) => {
    ...
    dispatch(deleteUser({id, indexList, indexCard: index}))
  }
                    ...
                    <CardHeader
                      action={
                        <IconButton aria-label="settings">
                          <Avatar sx={{ bgcolor: cUser.userId === usersCard[index] ? green[500] : red[500], width: "34px", height: "34px" }} aria-label="recipe">
